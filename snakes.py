...
class Snake:
    def __init__(self):
        ...
        self.body = [Square(-20, 0), Square(0, 0), Square(20, 0)] # body is a list of squares with (20,0)
        ...
        # this is what makes snake move
        #suppose aage move karna hai toh nextX=1 and nextY=0 and nextPosition=(20+20*1,0+20*0) i.e (40,0)

    def moveOneStep(self):
        if Square(self.nextposition[0], self.nextposition[1]) not in self.body: #let say nextposition(40,0) whixh is not in body list
            # attempt (unsuccessful) at collision detection
            self.body.append(Square(self.nextposition[0], self.nextposition[1])) # append (40,0)
            # moves the snake head to the next spot, deleting the tail
            del self.body[0]
            self.headposition[0], self.headposition[1] = self.body[-1].x, self.body[-1].y # -1 will fetch the last appended body i.e (40,0)
            #now in body we have=[(0,0),(20,0),(40,0)]
